...
int main()
{
	...
	*/
/*
	Image i4{Point{700,700},"kep.jpg"};
	win.attach(i4);
	win.wait_for_button();
	for(int i = 1; i <= 64; i++)
	{
	...
	*/
	...
	Vector_ref<Rectangle> bre;
	for(int i = 700; i >= 0; i=i-100)
	{
		for(int j = 0; j <= 700; i=i+100)
		{
			
			{
			bre.push_back(new Rectangle{Point{i,j}, 100, 100});
			}
	...
	{
	bre[i].set_fill_color(Color::blue);
	win.attach(bre[i]);
	}
	/*
	Rectangle a{Point{0, 0},Point{100, 100}};
	win.attach(a);
	*/
		win.wait_for_button();
